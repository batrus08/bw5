...
router.post('/', async (req,res)=>{
  try{
    ...
    if(update.callback_query?.data){
      ...
      if(data.startsWith('pick:')){ const picked=data.split(':')[1]; await answerCallbackQuery(update.callback_query.id, `Pilih: ${picked}`); await sendMessage(chatId, `✅ Kamu memilih ${picked}`); }
      else if(data.startsWith('sel:')){ const code=data.split(':')[1]; await answerCallbackQuery(update.callback_query.id); await sendMessage(chatId, `📦 Produk dipilih: <b>${code}</b>`, { parse_mode:'HTML' }); }
      else if(data.startsWith('confirm:')){ const invoice=data.split(':')[1]; const r=await confirmPaid(invoice); await sendMessage(chatId, r.ok?`✅ Confirmed ${invoice}`:`❌ ${r.error}`); }
      else if(data.startsWith('reject:')){ const invoice=data.split(':')[1]; const r=await rejectOrder(invoice); await sendMessage(chatId, r.ok?`❌ Rejected ${invoice}`:`❌ ${r.error}`); }
      else if(data.startsWith('invited:')){ const invoice=data.split(':')[1]; const r=await markInvited(invoice); await sendMessage(chatId, r.ok?`✅ Marked invited ${invoice}`:`❌ ${r.error}`); }
      else if(data.startsWith('reproof:')){ const invoice=data.split(':')[1]; await sendMessage(chatId, `🔁 Request new proof for ${invoice}`); }
      else if(data.startsWith('resend:')){ const invoice=data.split(':')[1]; await prisma.tasks.create({ data:{ order_id: (await prisma.orders.findUnique({ where:{ invoice } })).id, kind:'RESEND_INVITE' } }); await sendMessage(chatId, `🔁 Resend task queued for ${invoice}`); }
