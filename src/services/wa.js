...
async function waCall(path, body){
  ...
  return retry(async()=>{
    ...
    if (!res.ok){
      throw new Error(`WA ${res.status}: ${data.error?.message || 'unknown'}`);
    }
    return data;
  }, {
    attempts: 4,
    ...
    onError: async (e,i)=>{
      if (i===4){
        await prisma.deadletters.create({ data: { channel:'WHATSAPP', endpoint:path, payload: body, error: e.message, retry_count:i } });
...
function sendInteractiveButtons(to, bodyText, buttons){
  return waCall('messages', {
    ...
    interactive:{
      type:'button',
      body:{ text: bodyText },
      action:{ buttons: buttons.map((t,i)=>({ type:'reply', reply:{ id:`b${i+1}`, title:t } })) }
    }
  });
}
...
function sendListMenu(to, header, bodyText, sections){
  return waCall('messages', {
    ...
    interactive:{
      ...
      action:{
        button:'Pilih',
        sections: sections.map(s=>({ title:s.title, rows: s.rows.map(r=>({ id:r.id, title:r.title, description:r.desc||'' })) }))
      }
    }
  });
}

module.exports = { sendText, sendInteractiveButtons, sendListMenu };
