...
const { PAYMENT_DEADLINE_MIN } = require('../config/env');
...
async function createOrder({ buyer_phone, product_code, qty=1, amount_cents, email }){
  ...
  const deadline_at = new Date(Date.now() + PAYMENT_DEADLINE_MIN*60*1000);
  const order = await prisma.orders.create({
    data: { invoice, buyer_phone, product_code, qty, amount_cents: toCents(amount_cents), status:'PENDING_PAYMENT', email, deadline_at }
  });
...
async function setPayAck(invoice){
  ...
  await addEvent(upd.id, 'REMINDER_SENT', 'Waiting admin verification'); // reuse
...
async function confirmPaid(invoice){
  ...
  // create task depending on mode
  if (order.product.delivery_mode === 'privat_invite') {
    await prisma.tasks.create({ data:{ order_id: order.id, kind:'INVITE_CHATGPT', due_at: new Date(Date.now()+15*60*1000) } });
    await addEvent(order.id, 'INVITE_QUEUED', 'Invite task created', { kind:'INVITE_CHATGPT' });
  } else if (order.product.delivery_mode === 'canva_invite') {
    await prisma.tasks.create({ data:{ order_id: order.id, kind:'INVITE_CANVA', due_at: new Date(Date.now()+15*60*1000) } });
    await addEvent(order.id, 'INVITE_QUEUED', 'Invite task created', { kind:'INVITE_CANVA' });
  } else {
    // TODO: auto assign account if available
    await addEvent(order.id, 'DELIVERY_READY', 'Ready to deliver account');
  }
  return { ok:true, order:upd };
}
